...
def organize_files(downloads_path, study_path):
    ...
    for file in files:
        ...
        elif any(model in file_name for model in ["blackboxai", "anthropic", "openai", "cohere", "deepmind", "gemma", "groq", "phind"]):
            model_name = next((model for model in ["blackboxai", "anthropic", "openai", "cohere", "deepmind", "gemma", "groq", "phind"] if model in file_name), "")
        ...
        elif "python" in file_name or any(tool in file_name for tool in ["venv", "pip", "conda"]):
            target_folder = os.path.join(study_path, "programming", "python", "basics")
            shutil.copy2(os.path.join(downloads_path, file), target_folder)
            copied_files[target_folder].append(file)

        elif "ssh" in file_name:
            target_folder = os.path.join(study_path, "ssh")
            shutil.copy2(os.path.join(downloads_path, file), target_folder)
            copied_files[target_folder].append(file)

        elif "cabina" in file_name:
