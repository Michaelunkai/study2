...
def load_data():
    ...
    movies = movies.dropna(subset=['primaryTitle', 'averageRating', 'numVotes', 'genres', 'startYear'])

    # Convert startYear to string to avoid TypeError
    movies['startYear'] = movies['startYear'].astype(int).astype(str)

    # Sort and return the top 10,000 movies
    return movies.sort_values(by=['averageRating', 'numVotes'], ascending=[False, False]).head(10000)
...
class MainWindow(QMainWindow):
    def __init__(self, data):
        ...
        self.setWindowTitle("Top 10,000 IMDb Movies (15,000+ Votes)")
        ...
        # Add a title label
        title_label = QLabel("Top 10,000 IMDb Movies by Rating (15,000+ Votes)", self)
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("font-size: 24px; font-weight: bold; color: white; margin-bottom: 20px;")
        layout.addWidget(title_label)

        # Create a table to display the top 10,000 movies
        self.table = QTableWidget()
        self.table.setRowCount(len(data))
        self.table.setColumnCount(6)
        self.table.setHorizontalHeaderLabels(["Rank", "Movie Title", "Rating", "Votes", "Genre", "Year"])
