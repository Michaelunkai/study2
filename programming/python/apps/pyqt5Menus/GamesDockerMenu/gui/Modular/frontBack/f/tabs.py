from PyQt5.QtWidgets import (QWidget, QPushButton, QGridLayout, QScrollArea, 
                           QStackedWidget, QInputDialog, QMessageBox, QDialog,
                           QVBoxLayout, QListWidget, QListWidgetItem)
from PyQt5.QtCore import Qt
from backend import load_tabs_config, save_tabs_config, load_settings, save_settings
...
class TagContainerWidget(QWidget):
    ...
    def update_layout(self, buttons):
        ...
        row = 0
        col = 0
        for button, tag_info in buttons.items():
            if self.type_name == "all" and not self.should_display_tag(tag_info.get("category")):
                continue
...
class ExcludeTabsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Connect item change signal after adding items
        self.list_widget.itemChanged.connect(self.on_item_changed)
        
        self.setLayout(layout)

    def on_item_changed(self, item):
        # Update internal excluded tabs list without saving to disk
    ...
    def manage_exclusions(self):
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                # Refresh the main window's tag display
                self.parent.create_tag_buttons()
        except Exception as e:
