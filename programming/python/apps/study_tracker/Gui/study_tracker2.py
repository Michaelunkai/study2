...
def view_logs():
    conn = sqlite3.connect('study_tracker.db')
    c = conn.cursor()
    c.execute('SELECT * FROM study_logs ORDER BY date DESC, id DESC')
    logs = c.fetchall()
    conn.close()
    return logs
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if logs:
                current_date = None
                colors = [colorama.Fore.RED, colorama.Fore.GREEN, colorama.Fore.BLUE, colorama.Fore.YELLOW, colorama.Fore.MAGENTA, colorama.Fore.CYAN]
                color_index = 0
                for log in logs:
                    if log[1] != current_date:
                        current_date = log[1]
                        color_index = (color_index + 1) % len(colors)
                        print(f"\n{colors[color_index]}Date: {current_date}")
                    log_display = f"ID: {log[0]}, Subject: {log[2]}, Duration: {log[3]} minutes, Notes: {log[4]}"
                    print(f"{colors[color_index]}{log_display}")
            else:
                print("No logs found.")
