...
def get_system_info():
    ...
    try:
        info += "=== CPU Info ===\n"
        output = get_wmic_output(["cpu", "get", "name,NumberOfCores,NumberOfLogicalProcessors", "/format:list"])
        info += output + "\n"
    except Exception as e:
        info += f"WMIC error (CPU): {e}\n"
        try:
            query = "Get-CimInstance -ClassName Win32_Processor | Select-Object Name, NumberOfCores, NumberOfLogicalProcessors | Format-List"
            output = run_powershell_query(query)
            info += output + "\n"
        except Exception as e2:
