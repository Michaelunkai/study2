...
def shrink_file(input_file, output_file):
    if input_file.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.webm', '.wmv', '.flv', '.mpeg', '.mpg', '.3gp', '.m4v', '.vob')):
        cmd = [
            "ffmpeg", "-i", input_file,
    ...
    elif input_file.lower().endswith(('.mp3', '.wav', '.flac', '.aac', '.m4a')):
        cmd = [
            "ffmpeg", "-i", input_file,
            "-c:a", "aac", "-b:a", "128k",
            "-y", output_file
...
def main():
    ...
    supported_extensions = ('.mp4', '.avi', '.mov', '.mkv', '.webm', '.wmv', '.flv', '.mpeg', '.mpg', '.3gp', '.m4v', '.vob', '.mp3', '.wav', '.flac', '.aac', '.m4a')
    files = [f for f in os.listdir(path) if f.lower().endswith(supported_extensions)]

    if not files:
        print(f"No supported video or audio files found in the directory: {path}")
    ...
    max_workers = os.cpu_count() - 1 or 1  # Ensure at least 1 worker if cpu_count() returns 1
