...
cleanup_docker() {
    ...
    sudo apt-get remove -y docker docker-engine docker.io containerd runc docker-ce docker-ce-cli docker-buildx-plugin docker-compose-plugin 2>/dev/null || true
    ...
    sudo rm -f /usr/local/bin/docker-compose /usr/bin/docker-compose /usr/libexec/docker/cli-plugins/docker-compose || true
...
echo_info "Setting up the Docker repository..."
